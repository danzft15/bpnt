...
if ($conn->connect_error) {
  trigger_error('Database connection failed: '  . $conn->connect_error, E_USER_ERROR);
    ...
    $buffer = "id_user".$separator ."nama_user".$separator ."tlp_user".$separator ."email".$separator ."username".$separator ."password".$separator ."status".$separator; 
    $buffer .= $newline; 
    
  $sql="select `id_user`,`nama_user`,`tlp_user`,`email`,`username`,`password`,`status` from `$tbuser` order by `id_user` desc";
  $jum=getJum($conn,$sql);	
  if($jum>0){						
