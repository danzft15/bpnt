...
$pdf->Cell(5,1,'Id User','LRTB',0,'C',1);
$pdf->Cell(5,1,'nama_user','LRTB',0,'C',1);
$pdf->Cell(5,1,'tlp_user','LRTB',0,'C',1);
$pdf->Cell(5,1,'email','LRTB',0,'C',1);
$pdf->Cell(3,1,'username','LRTB',0,'C',1);
$pdf->Cell(3,1,'status','LRTB',0,'C',1);
$pdf->Ln();
$pdf->SetFont('Arial','','8');

for ($j=0;$j<$i;$j++){
  $pdf->Cell(1,1,$j+1,'LRTB',0,'L');         // no
  $pdf->Cell(5,1,$cell[$j][0],'LRTB',0,'L'); // id_user
  $pdf->Cell(5,1,$cell[$j][1],'LRTB',0,'L'); // nama_user
  $pdf->Cell(5,1,$cell[$j][2],'LRTB',0,'L'); // tlp_user
  $pdf->Cell(5,1,$cell[$j][3],'LRTB',0,'L'); // email
  $pdf->Cell(3,1,$cell[$j][4],'LRTB',0,'L'); // username
  $pdf->Cell(3,1,$cell[$j][5],'LRTB',0,'L'); // status
  $pdf->Ln();
}
$pdf->Output();
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
